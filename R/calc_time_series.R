#' @export
calc_time_series <- function(df_tweet = NULL, df_user_num = NULL, df_nab = NULL, option = "twitter", process = F) {
  if (option == "twitter") {
    df_ts <- calc_time_series_twitter(df_tweet = df_tweet, df_user_num = df_user_num, process = process)
  }

  if (option == "nab") {
    df_ts <- calc_time_series_nab(df_nab = df_nab, process = process)
  }
  return(df_ts)
}

calc_time_series_nab <- function(df_nab, process = F) {
  df_ts_nab <- df_nab %>%
    mutate(
...
calc_time_series_twitter <- function(df_tweet, df_user_num, process = F) {
  df_ts_tw <- df_tweet %>%
