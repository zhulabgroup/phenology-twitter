...
registerDoSNOW(cl)
df_nab_process_list<-
foreach (i = 1:nrow(site_year),
         .packages = c("tidyverse","lubridate","zoo")) %dopar% {
           idoi<-site_year$id[i]
  ...
  df_nab_filter %>% 
    ...
    right_join(data.frame(date=seq(date(paste0(yearoi,"-01-01")),date(paste0(yearoi,"-12-31")),by="day")) %>% 
                 mutate(doy=yday(date)) %>% 
                 mutate(id=idoi,
                        year=yearoi)
               , by=c("id","year", "doy", "date"))  %>% # add NA to gap dates
    arrange(date) %>% 
...
df_ts_compare_annual<-df_ts_compare %>% 
  ...
  summarise(mean_tweet=mean(tweet, na.rm = T),
            mean_pollen=mean(pollen, na.rm=T)) %>% 
  filter(is.finite(mean_tweet),
