v_year <- c(2012, 2013, 2014, 2015, 2016, 2018, 2019, 2020, 2021, 2022) %>% as.character()

if (!file.exists(.path$dat_compile)) {
  for (year in v_year) {
    v_month <- seq(1, 12) %>%
      as.character() %>%
      str_pad(2, "left", "0")
    ...
    ls_df_month <-
      foreach(
        ...
        v_day <- list.dirs(str_c(.path$dat_query, "CSV/", year, "/", month, "/"), recursive = F) %>%
          str_sub(-2)

        ls_df_day <- vector(mode = "list")
        for (day in v_day) {
          v_files <- list.files(str_c(.path$dat_query, "CSV/", year, "/", month, "/", day, "/"), pattern = ".csv", full.names = T)
    ...
    write_rds(df_year, str_c(.path$dat_query, "RDS/", year, ".rds"))
  ...
  for (year in v_year) {
    ls_df_year[[year]] <- read_rds(str_c(.path$dat_query, "RDS/", year, ".rds")) %>%
      mutate(year = year)
  }
  df_compiled <- bind_rows(ls_df_year)
  write_rds(df_compiled, .path$dat_compile)
} else {
