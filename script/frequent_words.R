...
text2dtm <- function(text) {
  ...
  docs <- tm::tm_map(docs, tm::removeWords, c("amp"))
  # Remove punctuations
...
allerg_perc <- df_clean %>%
  filter(str_detect(text, "allerg")) %>%
  nrow() / df_clean %>% nrow()
...
if (FALSE) {
  hw <- wordcloud2::wordcloud2(df_text_tidy %>% head(500))
